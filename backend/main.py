...
async def analyze_wildfire(request: WildfireRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # Fetch NASA satellite image if requested
        if request.generate_image:
            try:
                ...
                print(f"Fetching NASA satellite image for ({request.coordinates.lat}, {request.coordinates.lng}) with ID: {image_id}")
                from nasa_fetch_image import fetch_nasa_earth_image
                fetch_result = fetch_nasa_earth_image(
                    request.coordinates.lat,
                    request.coordinates.lng,
                    str(image_path)
                )
                if not fetch_result:
                    print(f"Warning: NASA image fetch failed for {image_id}")
                    response_data["image_id"] = None
            except Exception as e:
                print(f"Warning: Failed to fetch NASA image: {str(e)}")
                response_data["image_id"] = None
